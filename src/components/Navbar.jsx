...
//       { name: 'Project Descriptions', path: '/faculty-projects' }
//     ]
//   },
...
// import logo from '../assets/logo.png'
// import { useNavigate, Link, useLocation } from 'react-router-dom'
// import joinEeaPdf from '../assets/Join_EEA.pdf' // Update the path if needed
...
//       (item) => item.path === path || (item.dropdown && item.dropdown.some((d) => d.path === path))
//     )
//     if (currentItem) {
//       setActiveItem(currentItem.name)
//     }
...
//   return (
//     <div
//       className={`fixed top-0 left-0 right-0 w-full z-50 px-5 py-3 transition-all duration-300 ${
//         isDarkBackground
//           ? 'backdrop-blur-md bg-gray-900/90 shadow-lg'
//           : 'backdrop-blur-md bg-white/20 shadow-lg'
//       }`}
...
//                   isDarkBackground ? 'bg-blue-500/20' : 'bg-blue-500/10'
//                 } blur-lg opacity-0 group-hover:opacity-100 transition-all duration-300`}
//               ></div>
//               <img
//                 src={logo}
//                 alt="logo"
...
//         <button
//           onClick={() => handleNavClick(item)}
//           className="px-3 py-2 font-semibold text-[18px] transition-colors duration-300"
//         >
//           <span
//             className={`${
//               activeItem === item.name
...
//             }
//             className="px-3 py-2 font-semibold text-[18px] flex items-center gap-1 hover:text-blue-500 transition-colors duration-300"
//           >
//             {item.name}
//             <svg
...
//           {/* Dropdown menu */}
//           <div
//             className={`absolute left-0 mt-2 bg-white text-black rounded-lg shadow-lg w-48 z-50 transition-all duration-300 ${
...
//                 onClick={() => {
//                   setActiveItem(item.name)
//                   setOpenDropdown(null) // optional: close dropdown on link click
//                 }}
//               >
//                 {drop.name}
//               </Link>
...
//                       <summary className="px-4 py-3 cursor-pointer font-semibold hover:bg-gray-100">
//                         {item.name}
//                       </summary>
//                       <div className="pl-6">
//                         {item.dropdown.map((drop, i) => (
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const currentItem = navItems.find(
      (item) =>
        item.path === path ||
        (item.dropdown && item.dropdown.some((d) => d.path === path))
    )
    if (currentItem) {
      if (currentItem.dropdown) {
        const activeDrop = currentItem.dropdown.find((d) => d.path === path)
        if (activeDrop) setActiveItem(activeDrop.name)
      } else {
        setActiveItem(currentItem.name)
      }
    }
                  ...
                  >
                    {item.dropdown.map((drop, i) => (
                      <Link
                        ...
                        className="block px-4 py-2 hover:bg-gray-100"
                        onClick={() => {
                          setActiveItem(drop.name) // ✅ highlight the correct child
                      ...
                      <div className="pl-6">
                        {item.dropdown.map((drop, i) => (
                          <Link
                            ...
                            className="block px-4 py-2 hover:bg-gray-50"
                            onClick={() => {
                              setActiveItem(drop.name) // ✅ track correct child
                              setIsMenuOpen(false)
                            }}
                          >
                            {drop.name}
